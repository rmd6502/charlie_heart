...
#else
static const uint8_t BITNUM[] = { 1<<0, 1<<1,1<<2,1<<3,1<<4, };
...
void pinMode(uint8_t pin, enum Directions direction)
{
    ...
    if (direction != INPUT_PULLUP && pinModes[pin] == INPUT_PULLUP) {
        REG(PUE_REG[pin]) &= ~BITNUM[pin];
    }
#endif
    if (direction == OUTPUT) {
        REG(DDR_REG[pin]) |= BITNUM[direction];
    } else {
        REG(DDR_REG[pin]) &= ~BITNUM[direction];
#if defined(PUEA)
...
{
    return (REG(PIN_REG[pin]) & BITNUM[pin]) ? 1 : 0;
}
